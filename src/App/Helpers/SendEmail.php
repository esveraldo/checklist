...
class SendEmail{
	...
	public function enviar(){	
		...
		$headers .= "Content-type: text/html; charset=utf-8" . $this->quebraLinha;
                $headers .= 'Content-Transfer-Encoding: 8bit'. $this->quebraLinha;
		$headers .= "From: " . $this->remetente . $this->quebraLinha;
		$headers .= "Return-Path: " . $this->remetente . $this->quebraLinha;
                $headers .= "Data = " . date("d/m/Y H:i:s") . $this->quebraLinha;
                $headers .= 'X-Mailer: PHP v' . phpversion() . $this->quebraLinha;
		if(strlen($this->cc) > 0) 
